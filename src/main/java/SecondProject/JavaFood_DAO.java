		...
		try {
			...
			try {
				...
				if (q == 0) {
					try {
						this.pstmt = this.con
								.prepareStatement("insert into songhit values('" + id + "', 0,'" + s + "')");
				...
				this.con.prepareStatement("insert into songhit values('" + id + "','0','" + songnumber + "');")
						.executeUpdate();
				System.out.println("아이디값 생성");
			}

			this.pstmt = this.con
					.prepareStatement("SELECT * FROM songhit WHERE ID = '" + id + "' AND SONGNUMBER  = '" + s + "'");
			...
			this.pstmt = this.con.prepareStatement(
					"UPDATE songhit SET HIT = '" + a + "' WHERE ID = '" + id + "' AND SONGNUMBER = '" + s + "'");
			this.pstmt.executeUpdate();
			rs1.close();
			this.pstmt.close();
			this.con.close();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void song1addhit(String songnumber) {
		...
		try {
			...
			this.con.close();
			this.con = this.dataFactory.getConnection();
			this.con.prepareStatement("UPDATE GENRE SET HITS = '" + a + "' WHERE SONGNUMBER = '" + songnumber + "'")
					.executeUpdate();
			this.con.close();
		} catch (SQLException e) {
			e.printStackTrace();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void addId(login_DTO vo) {
		try {
			this.con = this.dataFactory.getConnection();
			this.pstmt = this.con.prepareStatement(
					"insert into login values('" + vo.getId() + "','" + vo.getPw() + "','" + vo.getNic() + "','"
							+ vo.getPn() + "','" + vo.getPhone() + "','" + vo.getEmail() + "',null)");
			this.pstmt.executeUpdate();
			this.pstmt.close();
			this.con.close();
		} catch (SQLException e) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void removeId(login_DTO vo) {
		try {
			this.con = this.dataFactory.getConnection();
			this.pstmt = this.con.prepareStatement("UPDATE login SET pwd = '" + vo.getPw() + "', nic = '" + vo.getNic()
					+ "', phone='" + vo.getPhone() + "', email='" + vo.getEmail() + "', img='" + vo.getMyimg()
					+ "' WHERE id = '" + vo.getId() + "'");
			this.pstmt.executeUpdate();
			this.pstmt.close();
			this.con.close();
...
//앨범수록곡 리스트 출력하는 메소드 >artistinfo.jsp에서 쓰이는 메소드
	 ...
	 */
	public List<AlbumDTO> listAlbum_artid(String artid) {
		List<AlbumDTO> listAlbum = new ArrayList<AlbumDTO>();
...
//		List li = new ArrayList();
		...
		try {
			...
			this.con.prepareStatement(
					"UPDATE GENRE SET album_add='" + album_add + "' WHERE SONGNUMBER ='" + songnumber + "'")
					.executeUpdate();
			con.close();

			this.con = this.dataFactory.getConnection();
			this.con.prepareStatement(
					"UPDATE GENRE SET artist_add='" + artist_add + "' WHERE SONGNUMBER ='" + songnumber + "'")
					.executeUpdate();
			con.close();
		} catch (SQLException e) {
			e.printStackTrace();
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public List<song_DTO> listSong() {
		...
		try {
			...
			String query = "SELECT * FROM ( SELECT rownum num, genre.* FROM ( SELECT RANK() OVER (ORDER BY FAMOUS DESC) AS RANKING, a.* FROM ( SELECT (HITS *1) + (LIKES * 1.5) AS FAMOUS, s.*  FROM Genre s ) a ) genre ) WHERE num >=? AND num <=?";
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   public int totalpaging() {
       ...
       try {
           this.con = this.dataFactory.getConnection();
           String chart = " SELECT count(*) cnt FROM  Genre";
           this.pstmt = con.prepareStatement(chart);
           ResultSet rs = pstmt.executeQuery();
           while (rs.next()) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public void addList(String title, String explain, String id) {
		...
		// 플레이 리스트 추가 쿼리문 작성
		String add_query = "INSERT INTO playList(PL_ID, ID2, PL_Title, PL_Explain)"
	...
	public void deleteList(int pl_id, String id) {
		...
		String del_query = "DELETE FROM playList_content" + " WHERE PL_ID = ?";
		...
		// 플레이 리스트 삭제 쿼리문 작성
		del_query = "DELETE FROM PLAYLIST" + " WHERE PL_ID = ?" + " AND ID2 = ?";
	...
	public List<PlayListDTO> loadPL(String id) {
		...
		// 쿼리문 작성
		String load_query = "SELECT * FROM playList" + " WHERE ID2 = ?" + " ORDER BY PL_ID DESC";
	...
	public List<PlayListDTO> loadPL(String id, int start, int end) {
		...
		// 쿼리문 작성
		String load_query = "SELECT rownum as rnum, * FROM playList" + " WHERE ID2 = ?" + "	AND rnum >= ? AND rnum <= ?"
				+ " ORDER BY PL_ID DESC";
	...
	public int pl_totalCount(String id) {
		...
		String countQuery = "SELECT count(*) cnt FROM playList" + "WHERE id = ?";
	...
	public void doDeleteSong(int PL_ID, int listNumber) {
		...
		// 쿼리문 작성
		String delSong_query = "DELETE FROM playList_Content" + " WHERE ListNumber = ?" + "	AND PL_ID = ?";
